...
if(btnLogin){btnLogin.addEventListener('click',async()=>{if(!auth)return;const {signInWithEmailAndPassword}=await import('https://www.gstatic.com/firebasejs/10.13.0/firebase-auth.js');if(loginStatus)loginStatus.textContent='登入中';try{await signInWithEmailAndPassword(auth,loginEmail.value,loginPassword.value);if(loginStatus)loginStatus.textContent=''}catch(e){if(loginStatus){let msg='登入失敗';const c=e&&e.code||'';if(c==='auth/invalid-email')msg='Email 格式錯誤';else if(c==='auth/user-not-found')msg='帳號不存在';else if(c==='auth/wrong-password')msg='密碼錯誤';else if(c==='auth/too-many-requests')msg='嘗試過多，請稍後再試';else if(c==='auth/network-request-failed')msg='網路連線失敗';else if(c==='auth/operation-not-allowed')msg='此登入方式未啟用';loginStatus.textContent=msg}}})}
